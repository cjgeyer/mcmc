...
\item \label{enum:rw-induced} We will
estimate $\E(|T|)$ using a random-walk Metroplis algorithm for the density
induced by setting \texttt{b=1} in the \texttt{morph.metrop} function.
\citet{johnson-geyer} demonstrate that a central limit does hold for these estimates.
\end{enumerate}

To demostrate this, we will estimate $\E(|T|)$ \texttt{n.sims =
  \Sexpr{n.sims}} times with each method, in each case using a Markov chain
\texttt{n.iter = \Sexpr{n.iter}} iterations long, and then examine the
normality of the resulting estimates.
