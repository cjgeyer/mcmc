...
whereas the function \texttt{lud.induced} is unvectorized,
mapping vectors to scalars.

Before we start using random numbers, we set the see of the random number
...
\label{fig:fig0}
\end{figure}
It looks like there is no significant autocorrelation among the batchs
...
\item \label{enum:rw-induced} Estimate the mean of the target distribution
  using a random-walk Metroplis algorithm implemented by the
...
We do the latter first, since we are already set up for it.
But we use an unbatched and longer run.
...
out$accept
@
A scale of 6 appears to be about right.  Now we do a similar long run for
this sampler.
...
par(oldpar)
@
makes time series plots for the two chains (Figure~\ref{fig:fig2}).
\begin{figure}
\begin{center}
...
@
\end{center}
\caption{Time series plots for morphed and unmorphed chains.}
\label{fig:fig2}
\end{figure}
Both appear stationary and it is hard to tell the difference from these
plots.  The range of the unmorphed plot appears wider but that is only
due to a few points and so may not be correct.
...
par(oldpar)
@
makes autocovariance plots for the two chains (Figure~\ref{fig:fig3}).
\begin{figure}
\begin{center}
...
@
\end{center}
\caption{Autocovariance plots for morphed and unmorphed chains.}
\label{fig:fig3}
\end{figure}
Again, not much apparent difference.
...
@
Again, not much difference.  Although we know the unmorphed chain
is much worse in theory, it seems to work in practice.
(More study is needed here, perhaps looking at longer runs.
But we don't want to take the time for really long runs in a vignette.)
...
@
\end{center}
\caption{Q-Q plot of batch means (batch length \Sexpr{blen})
for the unmorphed chain.}
\label{fig:fig4}
\end{figure}
Finally we see bad behavior of the unnmorphed chain.  These batch means
...
But on further thought, this is just heavy-tailedness of the target
distribution.  If one makes a Q-Q plot for means of sample size 100
for independent and identically distributed $t(3)$ random variables
one sees more or less the same thing.  One needs sample size 300
for approximate normality (as indicated by a Q-Q plot) for this distribution.
For MCMC we need about 300 times the lag at which autocorrelations become
insignificant, so we need batch length about 5000.  Let's try that.
<<batch-5000>>=
blen <- 5000
foo <- as.vector(out.unmorph$batch)
stopifnot(length(foo) %% blen == 0)
...
shapiro.test(foo)
@
not significantly different from normally distributed (although with such
a small number of batch means, perhaps that is just low power).

Since last issue we encountered (non-normality of batch means for moderate
but not really large batch length) is caused by the heavy-tailedness of
the target distribution, the morphed chain will have all the same issues.

This suggests that we should perhaps use a more robust estimator of scale
than the standard deviation of the batch means which using small batches
(length 100), but since there is nothing in the literature about this
(as far as we know), we will say no more about it.
...
deparmtnet, there is considerable overlap of the distributions of
...
<<cauchy-hessian>>=
hout <- optimHess(c(mu.hat, sigma.hat), lup)
hout
@
...
and it isn't (it is zero for $\sigma \le 0$).  We tried makeing $\log(\sigma)$
...
\newblock Necessary conditions for geometric and polynomial ergodicity of
    random-walk-type {M}arkov chains.
...
\newblock \emph{University of Minesota}. \url{http://purl.umn.edu/113140}
